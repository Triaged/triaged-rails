module Common::NotificationService
	...
	def self.push_message user, message
		alert = "#{message.author.name} is discussing a #{message.feed_item.provider_name.humanize} #{message.feed_item.event_type.name.humanize}"
		Common::NotificationService.push(user, message.feed_item.id, alert)
	end

	def self.push_feed_item user, item
		alert = "#{item.provider_name.humanize} #{item.event_type.name.humanize}"
		Common::NotificationService.push(user, item.id, alert)
	end

	def self.push_thumbsup thumber, item
		alert = "#{thumber.name.humanize} gave you a thumbs up!"
		Common::NotificationService.push(item.author, item.id, alert)
	end
